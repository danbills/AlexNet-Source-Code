"""
Max-Performance AlexNet ImageNet Training Engine in JAX/Flax NNX.
Optimizations:
1. Mixed Precision (bfloat16) on RTX 4090 Tensor Cores.
2. Direct Zero-Copy PyTorch -> JAX GPU Tensor Transfer via torch.utils.dlpack.
3. 16 CPU DataLoader Workers with C++ TorchVision libjpeg decoding.
4. TensorBoard Metric Logging.
"""
...
def main():
    ...
    print(f"=== Starting Mixed-Precision (bfloat16) AlexNet Training ===")
    print(f"Hyperparams: Batch Size = {args.batch_size}, LR = {args.base_lr}, Workers = {args.num_workers}")
    ...
    rngs = nnx.Rngs(0)
    model = AlexNet(num_classes=1000, dtype=jnp.bfloat16, rngs=rngs)
